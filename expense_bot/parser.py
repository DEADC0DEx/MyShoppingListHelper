...
הסכמה:
{{"action":"add_expense","merchant":"<שם בית העסק>","amount":<מספר>,"currency":"ILS","date":"<YYYY-MM-DD או null>","category":"<קטגוריה>"}}
...
- date: תאריך בפורמט YYYY-MM-DD, או null אם לא הוזכר
...
- "שלום, בכרטיסך [4 ספרות] אושרה עסקה ב-[DD/MM] בסך [סכום] ש"ח ב[עסק]" — ישראכרט
  → merchant: שם העסק שמופיע אחרי "ב" בסוף המשפט; date: השנה הנוכחית + DD/MM מהטקסט
- "בכרטיס מסטרקארד שמסתיים ב[ספרות] קיבלנו בתאריך [DD/MM] בקשה לעסקה בחו"ל ב[עסק] בסך [סכום] שח" — כאל
  → merchant: שם העסק; amount: הסכום בשקלים; date: השנה הנוכחית + DD/MM מהטקסט
...
- date: YYYY-MM-DD אם קיים בשורה, אחרת null
